...
public class JarLoader {
   ...
   * <li>the underlying VM neither supports agents nor uses URLClassLoader as its system class loader (extremely unlikely from
   * Java 1.6+).
   * </ul>
   *
  ...
  /**
   * Returns a string that describes the strategy being used to add JAR files to the class path. This is meant mainly to assist
   * with debugging and diagnosing client issues.
   *
   * @return returns {@code "none"} if no strategy was found, otherwise a short describing the method used; the value
   *         {@code "reflection"} indicates that a fallback not compatible with Java 9+ is being used
   */
   ...
   *
   * <p>
   * This method is automatically invoked when the JRE loads this class as an agent using the option
   * {@code -javaagent:jarPathOfThisClass}.
   *
   * <p>
   * For this to work the {@code MANIFEST.MF} file <strong>must</strong> include the line
   * {@code Premain-Class: ca.cgjennings.jvm.JarLoader}.
   *
   ...
   *
   * <p>
   * This method is called when the agent is attached to a running process. In practice, this is not how JarLoader is used, but it
   * is implemented should you need it.
   *
   * <p>
   * For this to work the {@code MANIFEST.MF} file <strong>must</strong> include the line
   * {@code Agent-Class: ca.cgjennings.jvm.JarLoader}.
   *
   * @param agentArgs       agent arguments; currently ignored
   * @param instrumentation provided by the JRE
   */
